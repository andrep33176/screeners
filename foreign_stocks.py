...
britain="http://www.nasdaq.com/screening/companies-by-region.aspx?region=Europe&country=United+Kingdom&render=download"
germany="http://www.nasdaq.com/screening/companies-by-region.aspx?region=Europe&country=Germany&render=download"	
netherlands="http://www.nasdaq.com/screening/companies-by-region.aspx?region=Europe&country=Netherlands&render=download"
japan="http://www.nasdaq.com/screening/companies-by-region.aspx?region=Asia&country=Japan&render=download"
singapore="http://www.nasdaq.com/screening/companies-by-region.aspx?region=Asia&country=Singapore&render=download"
israel="http://www.nasdaq.com/screening/companies-by-region.aspx?region=Middle+East&country=Israel&render=download"
canada="http://www.nasdaq.com/screening/companies-by-region.aspx?region=North+America&country=Canada&render=download"
...
frame=fetch_file(britain)
frame=frame.append(fetch_file(germany))
frame=frame.append(fetch_file(netherlands))
frame=frame.append(fetch_file(japan))
frame=frame.append(fetch_file(singapore))
frame=frame.append(fetch_file(israel))
frame=frame.append(fetch_file(canada))

nasurl="ftp://ftp.nasdaqtrader.com/SymbolDirectory/nasdaqlisted.txt"

nasdaq=pd.read_csv(nasurl,sep ="|")
nasdaq=nasdaq[nasdaq["Security Name"].str.contains("Common Stock",na=True)]
...
other=other[other["Security Name"].str.contains("Common Stock",na=True)]
...
countries=list(stocks['Country'])


stocks_dict={'Ticker':tickers,'Company':companies,'Industry':industries,'Country':countries}
...
stocks=stocks[['Company','Ticker','Industry','Country','Market_Cap','Market_Cap_Num','P/B','P/E','P/S','EPS','Dividend_Yield','PEG','Price','High']]
...
stocks=stocks[['Company','Ticker','Industry','Country','Market_Cap','DCF','DCF_Val','ROE','Debt_to_Earnings',\
		'P/B','P/E','GV','P/S','EPS','Debt_to_Equity','Dividend_Yield',\
		'Profit_Margin','PEG','Cash_Flow','Levered_Cash_Flow','CPS','Price','High','GN']]
stocks=stocks[(stocks['DCF_Val']>1)]


stocks.to_csv('foreign_stocks.csv',index=False)
t2=time.time()
print t2-t1
